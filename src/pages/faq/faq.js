...
const FAQs = ({ faqs }) => {
  ...
  const filteredFaqs = selectedCategory ? faqs.filter((faq) => faq.categoryNames.includes(selectedCategory)) : faqs;
        ...
        <Section>
          <Container>
            ...
            <div className={styles.filterButtons}>
              <button className={selectedCategory === '' ? styles.active : ''} onClick={() => setSelectedCategory('')}>
                Show all
              </button>
              <button
            ...
            <Accordion.Root className="AccordionRoot" type="single" defaultValue="item-1" collapsible>
              {faqs ? (
                filteredFaqs.map((faq, index) => (
                  <Accordion.Item key={faq.id} value={`tab${index + 1}`} className={styles.accordionItem}>
                    {console.log(faq)}
